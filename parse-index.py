...
def find_cray_mpich_gnu(index_df : pd.DataFrame) -> dict:
    names = index_df['Name']
    matches_indices = np.where(names.str.contains(r'cray-mpich-(?!.*(ucx))[0-9]+\.[0-9]+\.[0-9]+-gnu', regex=True))[0]
    if len(matches_indices) == 1:
        ...
        return index_df.iloc[matches_indices].assign(Version=match.group(1)).iloc[0].to_dict()
    raise Exception('Could not find cray-mpich-[version]-gnu-*')

def find_cray_mpich_nvhpc(index_df : pd.DataFrame) -> dict:
    names = index_df['Name']
    matches_indices = np.where(names.str.contains(r'cray-mpich-(?!.*(ucx))[0-9]+\.[0-9]+\.[0-9]+-nvidia', regex=True))[0]
    if len(matches_indices) == 1:
        cray_mpich_gnu = names.iloc[matches_indices[0]]
        pattern = r'cray-mpich-([0-9]+\.[0-9]+\.[0-9]+)'
        match = re.match(pattern, cray_mpich_gnu)
...
def find_cray_pals(index_df : pd.DataFrame) -> dict:
    names = index_df['Name']
    matches_indices = np.where(names.str.contains(r'cray-pals-[0-9]+\.[0-9]+\.[0-9]+-', regex=True))[0]
    if len(matches_indices) == 1:
...
def find_cray_pmi(index_df : pd.DataFrame) -> dict:
    ...
    names = index_df['Name']
    matches_indices = np.where(names.str.contains(r'cray-pmi(-devel|)-[0-9]+\.[0-9]+\.[0-9]+-', regex=True))[0]
    if len(matches_indices) == 2:
        ...
        return index_df.iloc[matches_indices].assign(Version=match.group(1)).to_dict(orient='records')
    raise Exception('Could not find cray-mpich-[version]-gnu-*')
...
def find_cray_mpich_gtl(index_df : pd.DataFrame, mpich_ver) -> dict:
    """Find cray-pmi and cray-pmi-devel"""
    names = index_df['Name']
    matches_indices = np.where(names.str.contains(f'cray-mpich-{mpich_ver}-gtl', regex=True))[0]
    if len(matches_indices) == 1:
...
lines = [(y['Name'], y['URL'], y['Version']) for y in [mpich_gnu, mpich_nvhpc, pmi, pmi_devel, mpich_gtl, pals]]
for line in lines:
    print(*line, end='\n')
